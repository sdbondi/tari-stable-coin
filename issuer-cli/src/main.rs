...
async fn main() {
    ...
    match cli.command {
        ...
        Command::WithdrawConfidential(com) => {
            
            com.run(client, cli.dump_buckets, cli.dry_run, cli.max_fee).await;
            
        }
        
        Command::WithdrawRevealed(com) => {
        ...
        Command::AddUserToDenyList(com) => {
            
            com.run(client, cli.dump_buckets, cli.dry_run, cli.max_fee).await;
            
        }
        
        Command::RemoveUserFromDenyList(com) => {
            
            com.run(client, cli.dump_buckets, cli.dry_run, cli.max_fee).await;
            
        }
        
        Command::CreateUserAccount(com) => {
            
            com.run(client, cli.dump_buckets, cli.dry_run, cli.max_fee).await;
            
        }
        
        Command::CheckTransfer(com) => {
